...
initiativeCards.forEach(card => {
    ...
    // Add partner organization content
    const title = card.querySelector('.partner-title').textContent;
    const sdgList = Array.from(card.querySelectorAll('.sdg-list li')).map(li => li.textContent);
    
    infoPopup.innerHTML = `
        <h3 style="color: #004d00; margin-bottom: 0.5rem;">${title}</h3>
        <p style="margin: 0.5rem 0; color: #333; font-size: 0.9rem;">
            Partner organization focused on sustainable development and community impact.
        </p>
        <div style="color: #333; font-size: 0.9rem;">
            <p style="font-weight: 600; margin: 0.5rem 0;">Focus Areas:</p>
            <ul style="padding-left: 1.2rem;">
                ${sdgList.map(sdg => `<li>${sdg}</li>`).join('')}
            </ul>
        </div>
    `;
...
searchInput.addEventListener('input', (e) => {
    ...
    initiativeCards.forEach(card => {
        ...
        const sdgTags = Array.from(card.querySelectorAll('.sdg-list li')).map(li => li.textContent.toLowerCase());
        
        const searchableText = `${title} ${sdgTags.join(' ')}`;
